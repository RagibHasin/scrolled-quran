...
impl<State, Action, ChildrenViews, F, G> VirtualHScroll<State, Action, ChildrenViews, F, G> {
    pub fn jump_to(mut self, anchor_index: Option<i64>) -> Self {
    ...
    G: Fn(Arg<'_, State>, Range<i64>) -> MessageResult<Action> + Send + Sync + 'static,
{
    ...
    fn build(&self, ctx: &mut ViewCtx, _: Arg<'_, State>) -> (Self::Element, Self::ViewState) {
        ...
        let pod = Pod::new(
            widgets::VirtualHScroll::new(self.anchor_index.unwrap_or(0))
                .with_valid_range(self.valid_range.clone())
    ...
    fn rebuild(
        ...
        {
            widgets::VirtualHScroll::overwrite_anchor(&mut element, idx);
        }

        let valid_range_changed = self.valid_range != prev.valid_range;
        if valid_range_changed {
            widgets::VirtualHScroll::set_valid_range(&mut element, self.valid_range.clone());
        }
